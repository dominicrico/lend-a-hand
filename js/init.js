...
(function($) {
	...
	skel.init({
		...
		breakpoints: {
			global: {
				range: '*',
				href: jekyll.baseurl + '/css/style.css',
				containers: 1400,
				grid: {
					gutters: 50
				}
			...
			wide: {
				range: '-1680',
				href: jekyll.baseurl + '/css/style-wide.css',
				containers: 1200,
				grid: {
					gutters: 40
				}
			...
			normal: {
				range: '-1280',
				href: jekyll.baseurl + '/css/style-normal.css',
				containers: 960,
				viewport: {
					scalable: false
				}
			...
			narrow: {
				range: '-980',
				href: jekyll.baseurl + '/css/style-narrow.css',
				containers: '95%',
				grid: {
					gutters: 30
				}
			...
			narrower: {
				range: '-840',
				href: jekyll.baseurl + '/css/style-narrower.css',
				grid: {
					collapse: 1
				}
			},
			mobile: {
				range: '-736',
				href: jekyll.baseurl + '/css/style-mobile.css',
				containers: '100%',
				grid: {
					gutters: 15,
					collapse: 2
			...
			if (skel.vars.IEVersion < 10) {
				$.fn.n33_formerize = function() {
					...
					_form.find('input[type=password]').each(function() {
						var e = $(this);
						var x = $($('<div>').append(e.clone()).remove().html().replace(
							/type="password"/i, 'type="text"').replace(/type=password/i,
							'type=text'));
						if (e.attr('id') !== '') x.attr('id', e.attr('id') +
							'_fakeformerizefield');
						if (e.attr('name') !== '') x.attr('name', e.attr('name') +
							'_fakeformerizefield');
